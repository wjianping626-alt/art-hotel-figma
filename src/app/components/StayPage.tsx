  ...
  return (
    <div className="w-full h-full overflow-y-auto" style={{ background: "#F4F0EA", fontFamily: "var(--font-sans)" }}>
      <div className="px-6 pt-14 pb-5">
        <p style={{ fontSize: "10px", color: "#A8673A", letterSpacing: "0.3em", fontWeight: 500 }} className="mb-3">ACT III · STAY</p>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "30px", fontWeight: 600, color: "#3C342C", lineHeight: 1.15 }}>选择您的<br />栖居之所</h1>
        <p style={{ fontSize: "13px", color: "#7A6E64", lineHeight: 1.7, marginTop: "12px", fontWeight: 300 }}>每间客房皆朝向麻黄梁峡谷，以黄土地貌为窗景，在慢境中安住一晚。</p>
      </div>
      <div className="px-4 flex flex-col gap-5 pb-28">
        {rooms.map((room, i) => (
          <motion.div key={room.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.12 }}
            className="rounded-3xl overflow-hidden" style={{ background: "#E9E2D8", border: room.featured ? "1.5px solid #A8673A" : "1px solid rgba(60,52,44,0.08)" }}>
            {room.featured && (
            ...
            <div className={`relative overflow-hidden ${room.featured ? "mx-4 rounded-2xl" : ""}`} style={{ height: "220px" }}>
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-right flex-shrink-0 ml-3">
                  <span style={{ fontFamily: "var(--font-inter)", fontSize: "22px", fontWeight: 600, color: "#3C342C" }}>{room.price}</span>
              ...
              <p style={{ fontSize: "12px", color: "#7A6E64", lineHeight: 1.75, marginBottom: "16px", fontWeight: 300 }}>{room.highlights}</p>
              <button onClick={() => setBooked(room.id)} className="w-full rounded-2xl py-3.5 transition-all duration-300 active:scale-[0.98]" style={{ background: booked === room.id ? "#2D2D2D" : "#A8673A", color: "#F4F0EA", fontSize: "14px", fontWeight: 600, letterSpacing: "0.08em", fontFamily: "var(--font-sans)" }}>
